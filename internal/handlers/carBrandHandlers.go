...
func UploadLogoHandler(s3Conf *configs.S3Config) gin.HandlerFunc {
	return func(ctx *gin.Context) {
		...
		// upload to S3
		result, err := s3Conf.Uploader.Upload(ctx, &s3.PutObjectInput{
		...
		carBrand.LogoImage = result.Location
		database.DB.Save(&carBrand)

		signedURL, err := utils.GetSignedUrl(ctx, s3Conf, s3Conf.BucketName, carBrand.LogoImage, 15*time.Minute)
		if err != nil {
			ctx.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
			return
		}
